...
public:
  ...
  constexpr error_code() noexcept : d1_(), lc_flags_(0) {}
  ...
  error_code(int val, const error_category &cat,
      ...
      : d1_(), lc_flags_((loc ? reinterpret_cast<boost::uintptr_t>(loc) : 2) |
                         +detail::failed_impl(val, cat)) {
    d1_.val_ = val;
    d1_.cat_ = &cat;
  }
  ...
  BOOST_SYSTEM_CONSTEXPR error_code(
      ...
      typename detail::enable_if<
          ...
          std::is_error_code_enum<ErrorCodeEnum>::value>::type * = 0) noexcept
      : d1_(), lc_flags_(0) {
  ...
  error_code(error_code const &ec, source_location const *loc) noexcept
    ...
    *this = ec;
    ...
    if (ec.lc_flags_ != 0 && ec.lc_flags_ != 1) {
      lc_flags_ = (loc ? reinterpret_cast<boost::uintptr_t>(loc) : 2) |
                  (ec.lc_flags_ & 1);
    }
  }

  error_code(std::error_code const &ec) noexcept : d1_(), lc_flags_(0) {
#ifndef BOOST_NO_RTTI

    if (detail::std_category const *pc2 =
            dynamic_cast<detail::std_category const *>(&ec.category())) {
  ...
  BOOST_SYSTEM_CONSTEXPR int value() const noexcept {
    if (lc_flags_ != 1) {
      ...
      std::error_code const &ec =
          *reinterpret_cast<std::error_code const *>(d2_);

      unsigned cv = static_cast<unsigned>(ec.value());
      unsigned ch = static_cast<unsigned>(
          reinterpret_cast<boost::uintptr_t>(&ec.category()) %
          2097143); // 2^21-9, prime
  ...
  operator bool() const noexcept // true if error
  {
    return failed();
  }
  ...
  source_location const &location() const noexcept {
    ...
    return lc_flags_ >= 4 ? *reinterpret_cast<source_location const *>(
                                lc_flags_ & ~static_cast<boost::uintptr_t>(1))
                          : loc;
  }
...
private:
  ...
  BOOST_SYSTEM_CONSTEXPR bool equals(int val,
                                     error_category const &cat) const noexcept {
    if (lc_flags_ == 0) {
...
public:
  ...
  operator==(const error_code &lhs, const error_code &rhs) noexcept {
    ...
    if (s1 && s2) {
      std::error_code const &e1 =
          *reinterpret_cast<std::error_code const *>(lhs.d2_);
      std::error_code const &e2 =
          *reinterpret_cast<std::error_code const *>(rhs.d2_);
  ...
  inline friend bool operator==(std::error_code const &lhs,
                                error_code const &rhs) noexcept {
    return lhs == static_cast<std::error_code>(rhs);
  }

  inline friend bool operator==(error_code const &lhs,
  ...
  template <class E, class N1 = void,
            class N2 = typename detail::enable_if<
                std::is_error_code_enum<E>::value>::type>
  BOOST_SYSTEM_CONSTEXPR inline friend bool operator==(error_code const &lhs,
                                                       E rhs) noexcept {
  ...
  template <class E, class N1 = void,
            class N2 = typename detail::enable_if<
                std::is_error_code_enum<E>::value>::type>
  BOOST_SYSTEM_CONSTEXPR inline friend bool
  operator==(E lhs, error_code const &rhs) noexcept {
  ...
  inline friend bool operator==(error_code const &lhs,
                                std::error_condition const &rhs) noexcept {
    return static_cast<std::error_code>(lhs) == rhs;
  }

  inline friend bool operator==(std::error_condition const &lhs,
                                error_code const &rhs) noexcept {
    return lhs == static_cast<std::error_code>(rhs);
  }
...
// This condition must be the same as the one in error_category_impl.hpp
  ...
  template <class T, class E = typename detail::enable_if<
                         detail::is_same<T, std::error_code>::value>::type>
  operator T &() {
    if (lc_flags_ != 1) {
  ...
  template <class T, class E = typename std::enable_if<
                         std::is_same<T, std::error_code>::value>::type>
...
inline bool operator==(const error_code &code,
  ...
  if (code.lc_flags_ == 1) {
    return static_cast<std::error_code>(code) ==
           static_cast<std::error_condition>(condition);
  } else {
...
inline std::size_t hash_value(error_code const &ec) {
  if (ec.lc_flags_ == 1) {
    std::error_code const &e2 =
        *reinterpret_cast<std::error_code const *>(ec.d2_);
