...
GLuint
Block::Mask(GLuint X, GLuint Y, GLuint Z, GLuint cent, GLuint normal, GLuint bltype, GLuint ac) {
  GLuint mask = 0;
  // oootttttnnnccczzzzzzyyyyyyxxxxxx 7 all add [block can have their final point at 32]
  //            xyz
...
std::vector<GLuint> Block::GenerateVerticies(GLuint ambient_occ) {
  ...
  vertices.push_back(Mask(x + 1, y + 1, z, 1, 0, blktype, (ambient_occ >> 6) & 7));  // 2
  ...
  vertices.push_back(Mask(x, y + 1, z + 1, 4, 1, blktype, (ambient_occ >> 2) & 7));  // 5
  // verticies.push_back({x - hside, y + hside, z + hside, x, y, z}); // 5 5
  vertices.push_back(Mask(x + 1, y + 1, z + 1, 0, 1, blktype, (ambient_occ >> 4) & 7));  // 6
  ...
  vertices.push_back(Mask(x, y + 1, z + 1, 4, 2, blktype, (ambient_occ >> 2) & 7));  // 5
  ...
  vertices.push_back(Mask(x + 1, y + 1, z + 1, 0, 3, blktype, (ambient_occ >> 4) & 7));  // 6
  // verticies.push_back({x + hside, y + hside, z + hside, x, y, z}); // 6 13
  vertices.push_back(Mask(x + 1, y + 1, z, 1, 3, blktype, (ambient_occ >> 6) & 7));  // 2
  ...
  // Top Face 5126
  vertices.push_back(Mask(x, y + 1, z + 1, 4, 4, blktype, (ambient_occ >> 2) & 7));  // 5
  ...
  vertices.push_back(Mask(x + 1, y + 1, z, 1, 4, blktype, (ambient_occ >> 6) & 7));  // 2
  // verticies.push_back({x + hside, y + hside, z - hside, x, y, z}); // 2 18
  vertices.push_back(Mask(x + 1, y + 1, z + 1, 0, 4, blktype, (ambient_occ >> 4) & 7));  // 6
