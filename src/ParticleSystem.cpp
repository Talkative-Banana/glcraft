...
ParticleSystem::ParticleSystem(const std::string& filepath) : m_effecttex(filepath) {
  m_quadva = std::make_unique<VertexArray>();
}

bool ParticleSystem::remove_effect(const uint32_t handle) {
  if (m_effects.find(handle) == m_effects.end()) return false;

  m_effects.erase(handle);
  return true;
}

uint32_t ParticleSystem::add_effect(std::unique_ptr<Effect> effect) {
