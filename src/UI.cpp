...
UI::UI() : m_quadva(std::make_unique<VertexArray>()) {}
...
uint32_t UI::add_component(UIComponent &&uicomp) {
  m_components.emplace(m_handle++, std::move(uicomp));
  return m_handle - 1;
}
...
void UI::Setup() {
  ...
  for (auto &[handle, component] : m_components) {
    ...
    for (auto index : Quad::faceindices)
      m_quad_indices.push_back(m_cnt + index);
    m_cnt += 4;
  }
  m_quadva->Bind();
...
void UI::Draw() {
  if (m_components.empty()) {
    std::cerr << "No components to draw";
    return;
  }
  m_quadva->Bind();
  m_vbo->Bind();
